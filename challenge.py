...
for i in range(0,n):
    ...
    all = record.split()  #split the string entered in order to store name and marks
    name = all[0]        #store the name of the student
    name = name.capitalize() # capitalize the name in order to store it in proper way
    marks = all[1:]          # store the marks as list
    mrks = [eval(j) for j in marks]  # convert the marks list from the string to integer
    res1 = arr.array('i', mrks)      # convert the marks as list  into an array where 'i' is the typecode
    for k in res1:
